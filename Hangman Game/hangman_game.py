...
def show_celebration_animation():
    ...
    canvas = tk.Canvas(celebration_window, width=400, height=400, bg='#FFF9C4')  # Light yellow
    canvas.pack()
...
# GUI Layout
word_label = tk.Label(root, textvariable=displayed_word, font=('Helvetica', 18), bg='#FFF9C4')
word_label.pack(pady=20)

attempts_label = tk.Label(root, text="Attempts left: ", bg='#FFF9C4')
attempts_label.pack()

attempts_left_label = tk.Label(root, textvariable=attempts_left, font=('Helvetica', 14), bg='#FFF9C4')
attempts_left_label.pack()

incorrect_guesses_label = tk.Label(root, text="Incorrect guesses: ", font=('Helvetica', 12), bg='#FFF9C4')
incorrect_guesses_label.pack(pady=10)

# Alphabet buttons for guessing letters
buttons_frame = tk.Frame(root, bg='#FFF9C4')
buttons_frame.pack()

# Create buttons for each letter
for letter in 'abcdefghijklmnopqrstuvwxyz':
    btn = tk.Button(buttons_frame, text=letter.upper(), width=4, height=2,
                    bg='#FFD54F',  # Golden yellow
                    fg='black',  # Black text
...
reset_button = tk.Button(root, text="Reset Game", command=reset_game, font=('Helvetica', 12), 
                         bg='#FFD54F', fg='black')  # Golden yellow
